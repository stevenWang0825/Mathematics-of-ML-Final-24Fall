...
class BiRNNTrainer:
    ...
    def train(self):
        ...
        if self.args.BiRNN_optim == "AdamW":
            self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.args.lr,weight_decay=self.args.decay)
    ...
    def save_and_plot(self):
        ...
        TESTING LR
        """
        os.makedirs("lr_logs", exist_ok=True)
        np.save(f"lr_logs/BiRNN_{self.args.lr}_{self.args.BiRNN_optim}.npy", logs)
        """
        TESTING DECAY
        """
        os.makedirs("decay_logs", exist_ok=True)
        np.save(f"decay_logs/BiRNN_{self.args.lr}_{self.args.decay}.npy", logs)
        """
        USE WHEN GENERATING SINGLE BIG PLOT
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--BiRNN_hid_dim", type=int, default=64)
